...
# https://github.com/Dao-AILab/flash-attention/tree/main/csrc/fused_dense_lib
# install fused_dense_lib
class FeedforwardNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, bias=True, fused_bias_fc=False, **factory_kwargs):
        super(FeedforwardNN, self).__init__()
        linear_cls = nn.Linear if not fused_bias_fc else FusedDense
        self.l1 = linear_cls(input_size,hidden_size, bias=bias, **factory_kwargs) # first layer
        self.relu = nn.ReLU() # activation function
        self.l2 = linear_cls(hidden_size, output_size, bias=bias, **factory_kwargs) # second layer
...
class MHA(nn.Module):
    ...
    def __init__(
        ...
        self.out_proj = FeedforwardNN(embed_dim, 2*embed_dim ,embed_dim, bias=out_proj_bias, fused_bias_fc=fused_bias_fc, **factory_kwargs) # by GPL
            ...
            if (
                ...
                if inference_params is None:
                    if not self.checkpointing:
                        ...
                        #context = self.inner_attn(qkv.to(torch.float16), **kwargs) #GPL
                    else:
