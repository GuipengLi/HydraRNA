...
def random_spans_noise_mask(noise_density, mean_noise_span_length , length, rng):
    ...
    def _random_segmentation(num_items, num_segments, rng):
        """
        Partition a sequence of items randomly into non-empty segmentsn
        Args:
...
class spanMaskTokensDataset(BaseWrapperDataset):
    ...
    def __getitem_cached__(self, seed: int, epoch: int, index: int):
        ...
        mask_idc0 = rng.choice(sz, num_mask, replace=False)
        mask_idc = np.where( random_spans_noise_mask( self.mask_prob, self.mean_noise_span_length , sz, rng) )[0]
        #print('mask_idc', mask_idc0, mask_idc)
